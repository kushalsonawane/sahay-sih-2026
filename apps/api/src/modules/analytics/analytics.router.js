...
router.get('/summary', async (req, res) => {
    ...
    if (req.user?.role === 'district_officer' && req.user.district)
        caseWhere.district = req.user.district;
    else if (req.user?.role === 'state_admin' && req.user.state)
        caseWhere.state = req.user.state;
    if (district)
        caseWhere.district = district;
    if (state)
        caseWhere.state = state;
    const [totalCases, highPriorityCases, riskCounts, checkInStats, interventionsThisWeek,] = await Promise.all([
    ...
    for (const r of riskCounts) {
        riskDist[r.riskLevel] = r._count.riskLevel;
    }
    res.json({
        success: true,
        data: {
...
router.get('/trend', async (req, res) => {
    ...
    for (const ci of checkIns) {
        ...
        if (!byDay[day])
            byDay[day] = { sum: 0, count: 0, highRisk: 0 };
